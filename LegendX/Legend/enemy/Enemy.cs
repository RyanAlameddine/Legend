...
namespace Legend.enemy
{
    public class Enemy
    {
        ...
        bool isAngry;
        TimeSpan timer;
        int random = GameApplication.rand.Next(4);
        TimeSpan randomtime;
        bool firsttime;
        int count;
        ...
        public virtual void Update(GameTime gameTime, Player p)
        {
            ...
            if (deathtimer <= TimeSpan.Zero)
            {
                ...
                else
                {
                    try
                    {
                        GameApplication.levellist[GameApplication.level - 1].enemies.Remove(this);
                    }
            ...
            {
                if (timer > randomtime) {
                    random = GameApplication.rand.Next(8);
                    randomtime = new TimeSpan(0, 0, GameApplication.rand.Next(1, 3));
                    timer = TimeSpan.Zero;
                    count++;
                    if (pos.X < 300)
                    {
            ...
            if (p.Hitbox.Intersects(Hitbox) && p.State != PlayerState.Interacting && speedoffset != Vector2.Zero)
            {
                ...
                GameApplication.healthManager.health -= damage / GameApplication.inventory.armour.defence;
                isAngry = true;
            }
            Hitbox.X = (int)((pos.X - (ori.X / 2)));
            Hitbox.Y = (int)((pos.Y - (ori.Y / 2)));
            pos += speed * speedoffset * GameApplication.deathspeed;
        ...
        public void hurt(int damage)
        {
            ...
            if (health <= 0 && speedoffset != Vector2.Zero)
            {
                ...
                GameApplication.levellist[GameApplication.level - 1].deadenemyparticle(deadparticles, 100);
                speedoffset = Vector2.Zero;
                draw = false;
                Item add = new Weapon("", GameContent.fourpixels, 0, WeaponPower.no, 0);
                for (int i = 0; i < itemdrops.Count; i++)
                {
                    if (GameApplication.rand.Next((int)itemdrops[itemdrops.Keys.ToList()[i]].X, (int)itemdrops[itemdrops.Keys.ToList()[i]].Y) == 0)
                    {
                ...
                if(add.name != ""){
                    GameApplication.levellist[GameApplication.level - 1].mobdropsonfloor.Add(new ItemOnFloor(add, pos, .2f, .001f, 0.08f, 6.25f));
                }
            }
        }
    }
