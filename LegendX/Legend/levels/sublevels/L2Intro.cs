    ...
    public class L2Intro :  Level
    {
        ...
        public override void Update(GameTime gameTime)
        {
            if (state == 1)
            {
                ...
                {
                    if (GameApplication.inventory.weapon.name == "Foam Sword")
                    {
                        state++;
                        GameApplication.inventory.AddItem(Items.GetItem("T-Shirt Armour"));
                    }
                }
            }
            else if (state == 2)
            {
                if (button.buttonpressed())
                {
                    if (GameApplication.inventory.armour.name == "T-Shirt Armour")
                    {
                        GameApplication.rendColor = new Color(0, 0, 0);
