...
namespace Legend.levels
{
    public class Intro
    {
        ...
        public void Update(GameTime gameTime)
        {
            ...
            {
                GameApplication.screen = Screens.Home;
            }
        ...
        void levelplus()
        {
            if (continuebool == false)
            {
                if (word != "")
                {
                    foreach (XmlElement e in GameApplication.xmlDoc.GetElementsByTagName("user"))
                    {
                    ...
                    if (error == false)
                    {
                        XmlElement characterElement = GameApplication.xmlDoc.CreateElement("user");
                        characterElement.SetAttribute("name", word);
                        characterElement.SetAttribute("level", "1");

                        XmlElement inventoryElement = GameApplication.xmlDoc.CreateElement("inventory");

                        characterElement.AppendChild(inventoryElement);

                        GameApplication.xmlDoc.DocumentElement.AppendChild(characterElement);
                        GameApplication.xmlDoc.Save(GameApplication.saveFile);

                        GameApplication.screen = Screens.Level;
                        GameApplication.name = word;
                    }
                }
            }
            else
                            ...
                            foreach (XmlElement elem in ((XmlElement)e.GetElementsByTagName("inventory")[0]).GetElementsByTagName("item"))
                            {
                                ...
                                if (elem.GetAttribute("equiptstatus") == "equipped.")
                                {
                                    ...
                                    if(elem.GetAttribute("type") == "Weapon"){
                                        GameApplication.inventory.weapon = (Weapon) tempitem;
                                    }else{
                                        GameApplication.inventory.armour = (Armour) tempitem;
                                    }
                                }
                                GameApplication.inventory.AddItem(tempitem);
                            }
                            GameApplication.level = int.Parse(e.GetAttribute("level"));
                            GameApplication.screen = Screens.Level;
                            GameApplication.name = word;
